...
def cross_check_apks(apks_metadata_per_paths):
    ...
    first_package_name = list(apks_metadata_per_paths.values())[0]['package_name']
    if PRODUCT.is_focus_flavor(first_package_name):
        _cross_check_focus_apks(apks_metadata_per_paths)
    elif PRODUCT(first_package_name) == PRODUCT.FENIX:
        _cross_check_fenix_apks(apks_metadata_per_paths)
    else:
        _cross_check_fennec_apks(apks_metadata_per_paths)


def _cross_check_focus_apks(apks_metadata_per_paths):
...
def _cross_check_fenix_apks(apks_metadata_per_paths):
...
def _cross_check_fennec_apks(apks_metadata_per_paths):
    _check_all_apks_have_the_same_package_name(apks_metadata_per_paths)
