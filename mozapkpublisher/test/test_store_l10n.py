...
def test_get_list_locales(monkeypatch):
    monkeypatch.setattr(
        ...
        if url == 'https://l10n.mozilla-community.org/stores_l10n/api/google/listing/beta/' else None
    )
...
def test_get_translation(monkeypatch):
    monkeypatch.setattr(
        ...
        } if url == 'https://l10n.mozilla-community.org/stores_l10n/api/google/translation/beta/en-GB/' else None
    )
...
def test_locale_mapping(monkeypatch):
    ...
    mock_json_url.side_effect = lambda url: {
        u'en-GB': u'en-GB',
        u'es-MX': u'es-US',
    } if url == 'https://l10n.mozilla-community.org/stores_l10n/api/google/localesmapping/?reverse' else None
